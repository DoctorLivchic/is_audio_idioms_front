...
async function getPhrase(){
  ...
  const phrase = await supabase.from('phrase_text').select('phrase_id').eq('phrase_text_text', firstText);
  return phrase;
...
async function translateFunction(){
  ...
  try{
    const translate = await supabase.from('phrase_text').select('phrase_text_text').eq('phrase_id', phrase.data[0]['phrase_id'])
