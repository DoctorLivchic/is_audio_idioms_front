...
async function translateFunction() {
  ...
  console.log("Выбранный фразеологизм: " + phrase.data[0]["phrase_text_text"]);
  try {
    document.getElementById("textAreaExit").value = phrase.data[0]["phrase_text_text"]; //то выводим во второй текстБокс перевод по выбранному языку к переводу
  } catch (error) {
