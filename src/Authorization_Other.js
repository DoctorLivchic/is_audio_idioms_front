...
export default function Authorization_Other() {
  ...
  async function addUser() {
    ...
    if (validName(name1, surname1, lastname)) {
          ...
          if (validPasport_ser(passport_series1)) {
            if (validPasport_id(passport_id1)) {
              if (validPhone(phone_number)) {
                  ...
                  try {
                    const { error } = await supabase
                      ...
                      .insert({
                        ...
                        surname: surname1,
                        patronymic: lastname,
                        passport_series: passport_series1,
                        passport_id: passport_id1,
                        date_of_birth: data1,
                        contact_number: phone_number,
                        diploma_id: diploma_id1,
                        email: email1,
                        password: password1,
                      });
                  } catch (error) {
                ...
                notification.open({
                  message: "Ошибка",
                  description: "Вы ввели некорректный номер телефона!",
                });
      ...
      notification.open({
        message: "Ошибка",
        description: "Вы ввели некорректное ФИО!",
      });
    ...
    if (index == -1) {
      ...
      if (data[index]["password"] == password) {
        ...
        if (data[index]["password"] != password) {
          ...
          notification.open({
            message: "Ошибка",
            description: "Вы ввели некорректный пароль!",
          });
        } else {
          notification.open({
            message: "Ошибка",
            description: "Вы выбрали некорректную роль!",
          });
        }
      }
    }
